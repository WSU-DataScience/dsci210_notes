...
![](img/h11/media/image1.png){width="5.129629265091864in"
height="1.4610673665791776in"}
...
![](img/h11/media/image2.png){width="4.749367891513561in"
height="2.351851487314086in"}
...
![](img/h11/media/image3.png){width="6.5in"
...
![](img/h11/media/image4.png){width="6.5in"
...
![](img/h11/media/image5.png){width="6.5in"
...
![](img/h11/media/image6.png){width="6.476851487314086in"
...
![](img/h11/media/image7.png){width="3.875in"
height="1.5276432633420822in"}
...
![](img/h11/media/image8.png){width="6.5in"
...
\$ unzip \*.zip
![](img/h11/media/image9.png){width="5.197916666666667in"
height="0.5075743657042869in"}
...
simple example below prints the values 1 through 5 to the screen.
  ...
  ------------------------------------------------------------------------------------------ ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
  ![](img/h11/media/image10.png){width="2.736111111111111in" height="0.855326990376203in"}   \$ for i in {1..5}
...
![](img/h11/media/image11.png){width="2.760728346456693in"
height="1.8657403762029747in"}

A quick glance to ensure all files have been successfully unzipped.

\$ ls –l

![](img/h11/media/image12.png){width="6.5in"
...
\$ wc –l \*.csv\
![](img/h11/media/image13.png){width="2.71875in"
height="0.9224332895888014in"}
...
![](img/h11/media/image14.png){width="6.5in"
height="0.791505905511811in"}

\$ tail Holidays.csv

![](img/h11/media/image15.png){width="6.5in"
height="0.7256791338582678in"}
...
![](img/h11/media/image16.png){width="6.5in"
...
![](img/h11/media/image17.png){width="6.5in"
height="0.6984831583552056in"}
...
![](img/h11/media/image18.png){width="5.138996062992126in"
height="0.8087325021872266in"}
...
3.  In Command \#3, the first \^ is for beginning of line, the \[\^:\]
    is syntex for not a colon, the \*: tells sed to grab any number of
...
![](img/h11/media/image20.png){width="6.5in"
height="0.5236111111111111in"}\
\
![](img/h11/media/image21.png){width="4.5in"
height="0.520673665791776in"}

The last step will be to add back in the header information. This can be
done using sed (Command \#1) or one could grab the first line from one
of the original files and then append it to the Holidays.csv file
(Command \#2). The output is being saved into a final called
Holidays\_Final.csv
...
![](img/h11/media/image22.png){width="6.478647200349957in"
height="0.71875in"}
...
![](img/h11/media/image23.png){width="6.5in"
...
![](img/h11/media/image24.png){width="6.5in"
...
![](img/h11/media/image25.png){width="6.5in"
...
![](img/h11/media/image26.png){width="4.90747375328084in"
height="2.4217541557305338in"}
...
-   Fields within the file are generically labelled \$1, \$2, etc. Thus,
    {print \$1, \$2, \$5, \$9, \$13, \$14, \$15} will print fields 1, 2,
    5, 9, 13, 14, and 15 to the output file.

\$ awk -F, -v OFS=, '{print \$1,\$2,\$5,\$9,\$13,\$14,\$15}'
HOlidays\_Final.csv &gt; Holidays\_LessFields.csv

The command above will extract fields 1, 2, 5, 9, 13, 14, and 15.

![](img/h11/media/image27.png){width="6.5in"
...
![](img/h11/media/image28.png){width="4.875in" height="1.825in"}
...
\$ awk -F, -v OFS=, '\$13\~"Customer" {print
\$1,\$2,\$5,\$9,\$13,\$14,\$15}' Holidays\_Final.csv &gt;
Holidays\_Customers.csv

Doing the same for the Subscriber rows.

Filter: usertype = “Subscriber”

\$ awk -F, -v OFS=, '\$13\~"Subscriber" {print
\$1,\$2,\$5,\$9,\$13,\$14,\$15}' HOlidays\_Final.csv &gt;
Holidays\_Subscribers.csv
...
ability to skip over certain rows. For example, if NR &gt; 1 is used,
then the 1^st^ row is skipped; however, the syntax in using this command
is beyond the scope presented here.
...
![](img/h11/media/image29.png){width="6.5in"
height="0.8965277777777778in"}

The following sed commands can be used to remove the quotes from the
first field, i.e. tripduration. Command \#1 remove the 2^nd^ quote on
each line and Command \#2 remove the 1^st^ quote from each line.
...
2.  What would happen if Command \#2 were run first, then Command \#1
    was run? Discuss.

A quick glance of the file after removing the quotes from the first
field.

![](img/h11/media/image30.png){width="6.5in"
height="0.9215277777777777in"}

The following awk command will keep lines whose tripduration, i.e. 1^st^
field, is less than 600.

\$ awk -F, -v OFS=, '\$1 &lt; 600 {print
\$1,\$2,\$5,\$9,\$13,\$14,\$15}' Holidays\_Final.csv &gt;
Holidays\_Lessthan10mins.csv

The tripduration &lt; 600 filter appear to have worked. Notice all
values in the 1^st^ field are less than 600.

![](img/h11/media/image31.png){width="6.5in" height="1.03125in"}
...
![](img/h11/media/image32.png){width="4.501930227471566in"
height="1.926786964129484in"}
...
![](img/h11/media/image33.png){width="4.571864610673666in"
height="2.875in"}
...
![](img/h11/media/image34.png){width="4.177606080489939in"
height="1.3837521872265968in"}
...
![](img/h11/media/image35.png){width="3.2604166666666665in"
height="1.6026902887139107in"}

![](img/h11/media/image36.png){width="4.359540682414698in"
...
![](img/h11/media/image37.png){width="5.489583333333333in"
...
![](img/h11/media/image38.png){width="2.25in"
...
0.39%.
  ...
  ![](img/h11/media/image39.png){width="2.3229166666666665in" height="0.6979166666666666in"}   (1/4)\*(1/4)\*(1/4)\*(1/4) = 0.0039 = 1/256
                                                                                               
                                                                                               ![](img/h11/media/image40.png){width="2.2395833333333335in" height="0.8478423009623797in"}
  -------------------------------------------------------------------------------------------- --------------------------------------------------------------------------------------------
  Probability of getting ANDY across all positions = 5 \* (1/256) = 5/256 = 0.0195 ≈ 2%
  
  ![](img/h11/media/image41.png){width="2.46875in" height="1.4583333333333333in"}
  -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
![](img/h11/media/image42.png){width="1.2389457567804025in"
...
-   Sed command deletes all characters beyond the first four, this
    output is piped forward to grep

-   Grep command searches for string ANDY. If ANDY is present in this
    line, the line is pushed forward

-   The wc -l simply counts the number of lines, i.e. the number of
    times ANDY is found in the first four characters.

1.  Run command \#1 and command \#2 in Bash. How many times did ANDY
    show up in the first four positions? Compute the empirical
    probability, i.e. \# times / 1000000. Is the empirical probability
    close to the theoretical value of 1/256?
...
4.  Consider Command \#3 below.
    ...
    Command \#3: \$ cat /dev/urandom | tr -dc 'ANDY\#\[0-3\]' | fold -w
    8 | head -1000000 &gt; random.txt

    This command works but is \*not\* what is needed here. Fix this
    command so that is produces random passwords as requested.

    ![](img/h11/media/image43.png){width="1.3854166666666667in"
    height="0.955097331583552in"}
...
-   Command \#4 generates 10,000,000 random passwords. Each password is
    8 characters long and is made up of the characters A, N, D, and Y.
...
-   Command \#6 uses grep to search for ANDY in each password. Again,
    the length of time for processing can be computed with date; placed
