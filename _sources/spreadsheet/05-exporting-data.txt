...
As an example of inconsistencies in data storage, do you remember how we
...
2. In the 'Format' field, from the list, select 'Comma Separated Values'
   (``*.csv``).
3. Double check the file name and the location where you want to save it
   and hit 'Save'.

An important note for backwards compatibility: you can open CSV files in
Excel!

.. figure:: ../fig/excel-to-csv.png
...
parse the ":raw-latex:`\r`" it will interpret your CSV file differently:
...
1. When exporting from Excel, save as a “Windows comma separated (.csv)”
   file
2. If you store your data file under version control using Git, edit the
...
-  this equates to replacing a (simple but manual) export to ``csv``
   with additional complexity/dependencies in the data analysis R code
...
If you are working with data that contains commas, you likely will need
...
page <http://www.datacarpentry.org/spreadsheet-ecology-lesson/discuss/>`__.
